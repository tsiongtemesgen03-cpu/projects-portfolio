...
<style>
/* FORCE GOLD BUTTON IN SIMULATOR */
.get-price-btn, 
...
.get-price-btn:hover,
.simulator-section button:hover,
button[type="submit"]:hover {
    background: linear-gradient(135deg, #c9a227, #a8841f) !important;
...
<style>
/* OLD STYLE OVERRIDDEN */ button, .btn-primary, .get-price-btn {
    background: linear-gradient(135deg, #d4af37, #b8962e) !important;
    color: white !important;
    border: none !important;
}
button:hover, .btn-primary:hover, .get-price-btn:hover {
    background: linear-gradient(135deg, #c9a227, #a8841f) !important;
}
</style>
...
def _build_ai_pricing_result(payload, fm):
    ...
    if demand == 'VERY HIGH':
        explanation_parts.append(f"Very high demand because the event is only {payload.get('distance_to_event_km', 0):.1f} km away")
    elif demand == 'HIGH':
        explanation_parts.append(f"High demand because the event is only {payload.get('distance_to_event_km', 0):.1f} km away")
    elif demand == 'MEDIUM':
        explanation_parts.append(f"Moderate demand with the event {payload.get('distance_to_event_km', 0):.1f} km away")
    else:
        explanation_parts.append("Lower event-driven demand due to distance from the venue")

    if comp['count'] > 0:
        explanation_parts.append(f"Comparable listings average ${comp['avg_price']:.0f}/night")

    if cluster['count'] > 0:
        explanation_parts.append(f"Similar {cluster['cluster_label']} listings are priced around ${cluster['avg_price']:.0f}/night")

    if action == 'RAISE':
        explanation_parts.append("The simulator recommends raising your price to stay aligned with market demand")
    elif action == 'DISCOUNT':
        explanation_parts.append("The simulator recommends discounting to stay competitive with nearby listings")
    else:
        explanation_parts.append("The simulator recommends holding the price near the current market level")
...
.sim-insight,.sim-explanation{font-size:12px;color:rgba(255,255,255,.5);margin-top:8px;max-width:500px;line-height:1.5;white-space:pre-line}
...
body{background:#f1efe9 !important;color:var(--pw-text)!important}
.blobs{display:none!important}
.app-shell{display:grid;grid-template-columns:280px 1fr;min-height:100vh;position:relative}
.side-rail{background:linear-gradient(180deg,#171717 0%,#232323 45%,#2b2b2b 100%) !important;border-right:1px solid rgba(255,255,255,0.06);padding:26px 18px 20px;position:sticky;top:0;height:100vh;display:flex;flex-direction:column;gap:18px}
.side-brand{display:flex;align-items:center;gap:12px;padding:8px 8px 18px;font-weight:800;font-size:22px;color:#f8fafc !important;letter-spacing:-.03em}
...
.side-sub{font-size:11px;font-weight:700;color:#cbd5e1 !important;text-transform:uppercase;letter-spacing:.11em;padding:8px 12px 2px}
.side-nav{display:flex;flex-direction:column;gap:6px}
.side-link{display:flex;align-items:center;gap:12px;padding:12px 14px;border-radius:16px;color:#dbe4f0 !important;font-size:14px;font-weight:600;transition:.18s background,.18s color,.18s transform;text-decoration:none}
.side-link:hover{background:rgba(255,255,255,0.08) !important;color:#ffffff !important}
.side-link.active{background:rgba(255,255,255,0.14) !important;color:#ffffff !important;box-shadow:inset 0 0 0 1px rgba(255,255,255,0.08) !important}
...
<style id="cece-final-theme-override">
:root{
  --bg:#f1efe9 !important;
  --bg2:#ebe7df !important;
  --pw-sidebar:#1f1f1f !important;
}
html, body{
...
.side-rail{
  background:linear-gradient(180deg,#171717 0%,#232323 45%,#2b2b2b 100%) !important;
...
<style id="pw-hero-charcoal-only">
.pw-dashboard-top .card.pw-hero{
    background: linear-gradient(135deg, #101010 0%, #1b1b1b 52%, #2a2a2a 100%) !important;
}
.pw-dashboard-top .card.pw-hero .pw-hero-title,
.pw-dashboard-top .card.pw-hero .pw-hero-copy,
.pw-dashboard-top .card.pw-hero .pw-eyebrow,
.pw-dashboard-top .card.pw-hero .pw-price-block small,
.pw-dashboard-top .card.pw-hero .pw-price-block strong,
.pw-dashboard-top .card.pw-hero .pw-arrow,
.pw-dashboard-top .card.pw-hero .pw-chip,
.pw-dashboard-top .card.pw-hero .pw-btn{
...
.pw-dashboard-top .card.pw-hero .pw-green-text{
...
<div class="app-shell">
  <aside class="side-rail">
    <a href="/" class="side-brand" aria-label="PriceWise home">
      <img src="/static/pricewise.png" alt="PriceWise logo" class="side-brand-logo">
